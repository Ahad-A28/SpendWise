...
const ensureCategoryExists = async (categoryName, userId) => {
    ...
    if (!cats[categoryName]) {
        ...
        await AppSetting.findOneAndUpdate({ key: 'categories', userId }, { key: 'categories', value: cats, userId }, { upsert: true, new: true });
    }
};
const tools = [
    {
...
const getOrCreateCreditConfig = async (userId) => {
    ...
    let data = typeof setting.value === 'object' ? setting.value : {
    ...
    try {
        ...
        if (data.credits < creditCost) {
            ...
            if (fileData) {
                ...
                if (creditCost > defaultCost)
                    costMessage += ` but now ${creditCost} increased due to high demand`;
                else if (creditCost < defaultCost)
                    costMessage = `Uploading a file costs ${creditCost} credit(s)`;
            }
            else {
                ...
                if (creditCost > defaultCost)
                    ...
                    costMessage = `${isAgentMode ? 'Agent Mode' : 'Normal Chat'} costs ${creditCost} credit(s)`;
            }
        ...
        const model = genAI.getGenerativeModel({
            ...
            tools: isAgentMode ? tools : undefined,
        });
            ...
            if (functionCalls && functionCalls.length > 0) {
                for (const call of functionCalls) {
                    if (call.name === 'addExpense') {
                        const { title, amount, category, paymentMethod } = call.args;
                        ...
                        const { title, targetAmount, deadline, autoSaveAmount } = call.args;
                        ...
                        const { category, allocated } = call.args;
                        ...
                        await Budget.findOneAndUpdate({ category, userId }, { category, allocated: budgetAmt, userId }, { upsert: true, new: true });
                        actionExecuted = `Added Category & Set Budget: ${category} set to ₹${budgetAmt}`;
                    }
                    else if (call.name === 'bulkAddExpenses') {
                        const { expenses: extractedExpenses } = call.args;
