...
export const getCategories = async (req, res) => {
    try {
        ...
        if (setting && setting.value && Object.keys(setting.value).length > 0) {
            ...
            await AppSetting.findOneAndUpdate({ key: 'categories', userId }, { key: 'categories', value: defaults, userId }, { upsert: true, new: true });
            res.json(defaults);
        }
    }
    catch (error) {
...
export const saveCategories = async (req, res) => {
    try {
        ...
        await AppSetting.findOneAndUpdate({ key: 'categories', userId }, { key: 'categories', value: data, userId }, { upsert: true, new: true });
