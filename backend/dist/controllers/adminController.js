...
export const getAdminUsers = async (req, res) => {
    try {
        ...
        if (invalidIds.length > 0) {
            ...
            await Expense.deleteMany({ userId: { $in: invalidIds } });
            await Budget.deleteMany({ userId: { $in: invalidIds } });
            await Goal.deleteMany({ userId: { $in: invalidIds } });
            await CreditLog.deleteMany({ userId: { $in: invalidIds } });
        }
        ...
        const usersData = validSettings.map(setting => {
            const data = typeof setting.value === 'object' ? setting.value : {
...
export const updateUserAdminSettings = async (req, res) => {
    try {
        ...
        let details = '';
        if (action === 'setLocked')
            details = `Locked status set to ${value}`;
        else if (action === 'addCredits')
            details = `Added ${value} credits`;
        else if (action === 'setCredits')
            details = `Set credits to ${value}`;
        else if (action === 'setChatCost')
            details = `Set Chat Cost to ${value}`;
        else if (action === 'setAgentCost')
            details = `Set Agent Cost to ${value}`;
        else if (action === 'setFileCost')
            details = `Set File Cost to ${value}`;
        else if (action === 'resetRateLimit')
            details = `Reset rate limit`;
